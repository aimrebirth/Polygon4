...
#undef ADD_LANGUAGE
    ...
    LangWidget = SNew(cb_type)
                     ...
                     .OnSelectionChanged_Lambda([this](const cb_item_type& InItem, ESelectInfo::Type type) {
                         polygon4::getCurrentLocalizationId(InItem->id);
                         CurrentLangItem->SetText(InItem->Text);
                     })
                     .OnGenerateWidget_Lambda([](const cb_item_type& InItem) { return SNew(STextBlock).Text(InItem->Text).Font(FSlateFontInfo(RobotoFont, 20)); })
                     .Content()[SAssignNew(CurrentLangItem, STextBlock).Text(SelectedItem->Text).Font(FSlateFontInfo(RobotoFont, 20))];
                     ...
                     [SNew(SHorizontalBox)
                      ...
                      + SHorizontalBox::Slot()
                            ...
                            .AutoWidth()[SNew(SVerticalBox) +
                                         SVerticalBox::Slot()[SAssignNew(MenuVB, SVerticalBox) + MainMenuButton(LOCTEXT("NewGameButtonLabel", "New Game"), &SMainMenu::OnNewGame) +
                      ...
                      + SHorizontalBox::Slot()
                            .VAlign(VAlign_Fill)
                            .HAlign(HAlign_Fill)[SNew(SVerticalBox) +
                                                 SVerticalBox::
                                                     Slot()[SAssignNew(ModsVB, SVerticalBox)
                                                            // label
                                                            + SVerticalBox::Slot()
                                                                  .VAlign(VAlign_Top)
                                                                  .HAlign(HAlign_Center)
                                                            ...
                                                            // List View
                                                            + SVerticalBox::Slot().VAlign(VAlign_Fill).HAlign(HAlign_Fill).Padding(Padding)[SAssignNew(ModsListView, SModListView)]
                                                            // Reload Button
                                                            + SVerticalBox::Slot()
                                                                  .VAlign(VAlign_Bottom)
                                                                  .HAlign(HAlign_Center)
                                                                  .Padding(Padding)
                                                                  .AutoHeight()[SNew(SButton).OnClicked(
                                                                      this, &SMainMenu::OnReloadMods)[SNew(STextBlock)
                                                                                                          .ShadowColorAndOpacity(FLinearColor::Black)
                                                                                                          .ColorAndOpacity(FLinearColor::White)
                                                                                                          .ShadowOffset(FIntPoint(-1, 1))
                                                                                                          .Font(FSlateFontInfo(RobotoFont, 40))
                                                                                                          .Text(LOCTEXT("ReloadModsButtonLabel", "Reload mod list"))]]] +
                                                 SVerticalBox::Slot()[SAssignNew(SavedGamesVB, SVerticalBox).Visibility(EVisibility::Collapsed)
                                                                      // label
                                                                      + SVerticalBox::Slot()
                                                                            .VAlign(VAlign_Top)
                                                                            .HAlign(HAlign_Center)
             ...
             + SVerticalBox::Slot()
                   ...
                   .AutoHeight()[SNew(SHorizontalBox)
                                 ...
                                 + SHorizontalBox::Slot()
                                       ...
                                       .HAlign(HAlign_Left)
                                       .AutoWidth()
                                       .Padding(Padding)[SNew(STextBlock)
                                                             .ShadowColorAndOpacity(FLinearColor::Black)
                                 ...
                                 // lang selector
                                 + SHorizontalBox::Slot().VAlign(VAlign_Center).HAlign(HAlign_Fill).AutoWidth()[LangWidget.ToSharedRef()]
                                 // space
...
template <typename F> SVerticalBox::FSlot& SMainMenu::MainMenuButton(FText Text, F function) const
{
